...
echo "[">>vis_im2txt.json
imageNum=0
while IFS='' read -r line || [[ -n "$line" ]]; do
	echo "{" >>vis_im2txt.json
	echo \"caption\">>vis_im2txt.json
	echo ":">>vis_im2txt.json
    echo \"$line\">>vis_im2txt.json
	echo "," >>vis_im2txt.json
	echo \"image_id\">>vis_im2txt.json
	echo ":" >>vis_im2txt.json
	imageNum=$((imageNum+1))
	echo $imageNum >>vis_im2txt.json
	echo "},">>vis_im2txt.json
	
done < "$1"
sed -i '$ s/.$//' vis_im2txt.json
echo "]">>vis_im2txt.json
